...
- `200 image/svg+xml` — **การ์ดเต็มใบ 420×600** (deterministic) ประกอบด้วย: กรอบโลหะตามระดับความหายาก (ทอง = MYTHIC), แถบชื่อไทย/อังกฤษ, ตราธาตุ, ดาวระดับ, ช่องภาพ, กล่องคำบรรยายคุณสมบัติ (สกิล/คำอธิบาย/lore), แถบ ATK/DEF/HP/SPD/MP และโฮโลแกรมสำหรับ RARE ขึ้นไป
- `?mode=overlay` — วาดเฉพาะกรอบ/ข้อความ เว้นช่องภาพโปร่งใส (ใช้ซ้อนทับภาพ AI ของการ์ด)
- ใช้กับ `<img>` ได้ตรง ๆ · คอมโพเนนต์ `CardFace` ในฝั่งเว็บทำการซ้อน 2 เลเยอร์ให้อัตโนมัติ

### `GET /api/cards/[id]/art` — ภาพวาด AI ของการ์ด
- `200 image/jpeg|png|webp` — ไฟล์ภาพที่สร้างไว้ในเครื่อง (`var/card-art/`) · แคช 1 วัน
- `404` ถ้าการ์ดใบนี้ยังไม่มีภาพ AI (ฝั่งเว็บจะถอยไปใช้การ์ดวาดเอง)
- สร้าง/สร้างใหม่: `npm run images:generate` (`--all`, `--limit N`, `--ids ...`, `--dry`)
- **prompt ที่ใช้:** deterministic จากข้อมูลการ์ด (ธาตุ/บทบาท/ระดับความหายาก/lore) ตาม GDD §10.2 · ทุก prompt ผ่าน `isPromptSafe()` ก่อนส่งออก
