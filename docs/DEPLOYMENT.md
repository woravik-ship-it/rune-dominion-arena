...
### ตรวจว่า "หน้าเว็บจริง" แสดงรูปการ์ดถูกต้อง (Phase 14.1)

```bash
# ต้องมี session token ของผู้เล่น (mint จากสคริปต์ภายในหรือใช้ cookie จากเบราว์เซอร์)
node scripts/inspect-cards-page.mjs --token "<session>" --path /cards --out /tmp/cards.png
# → พิมพ์ JSON: h1, จำนวนการ์ด, จำนวนรูปที่ "มองเห็นได้จริง" (invisible ต้องเป็น 0) + ถ่ายภาพหน้าจอ
```

ใช้เมื่อแก้ layout ของการ์ด: **ถ้า `invisible > 0` แปลว่ารูปการ์ดแสดงไม่ได้** (เคสจริงที่เคยเกิด: `CardFace` มีทั้งคลาส
`relative` และ `absolute` พร้อมกัน → เบราว์เซอร์เลือก `relative` → กล่องสูง 0px → รูปทุกใบ invisibles ทั้งหมด)

**ข้อกำหนดของ layout:** คอมโพเนนต์ `CardFace` ไม่ตั้ง `position` เอง — ผู้เรียกต้องวางในกล่องที่
`position: relative` + มีสัดส่วนการ์ด (`aspect-[7/10]`) หรือความสูงชัดเจน
